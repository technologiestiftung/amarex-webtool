...
def process_input_without_return(cdll, input_array):
    """
    Process an input array, without returning the output from C++
    Here there is no conversion back from a C++ vector to a Python array.
    ...
    # Define argument and return types for the C++ function processInput
    cdll.processInput.argtypes = [POINTER(c_int), c_int]
    cdll.processInput.restype = c_void_p
    ...
    # Process the input with C++
    cdll.processInputWithoutReturn((c_int * len(input_array_flat))(*input_array_flat), n_rows)
